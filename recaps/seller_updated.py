...
class Seller:
    ...
    def get_seller_delay_wait_time(self):
        ...
        def delay_to_logistic_partner(d):
            days = np.mean(
                (d.shipping_limit_date - d.order_delivered_carrier_date)
                / np.timedelta64(24, "h")
            )
            if days < 0:
                return abs(days)
    ...
    def get_active_dates(self):
        ...
        return (
            orders.merge(self.matching_table[["seller_id", "order_id"]], on="order_id")
            .groupby("seller_id")
            .agg({"date_first_sale": min, "date_last_sale": max})
        )
    ...
    def get_review_score(self):
        ...
        # Since same seller can appear multiple times in the same order,
    ...
    def get_quantity(self):
        ...
        n_orders = (
            matching_table.groupby("seller_id")["order_id"].nunique().reset_index()
        )
        n_orders.columns = ["seller_id", "n_orders"]

        quantity = matching_table.groupby("seller_id", as_index=False).agg(
            {"order_id": "count"}
        )
    ...
    def get_training_data(self):
        ...
        # Revenue and profits for Olist
        commission_revenue = training_set.sales / 10
        membership_revenue = (
            np.ceil(
                (training_set.date_last_sale - training_set.date_first_sale)
                / np.timedelta64(1, "M")
            ).map(lambda x: max(1, x))
            * 80
        )
        training_set["revenue"] = commission_revenue + membership_revenue
        training_set["profits_before_it_costs"] = (
